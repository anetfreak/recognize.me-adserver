...
public class AdDaoImpl implements AdDao {
	...
	public void saveAd(Ad ad) throws SQLException {
		try {
			...
			Object[] args = new Object[] { ad.getId(), ad.getName(),
					ad.getUrl(), ad.getContentType().getId(),
					ad.getBrand().getId(), ad.getRegion(), ad.getLanguage(),
					ad.getContent(), ad.getCreatedDate(), ad.getExpiryDate(),
					ad.getCategory().getId() };

			int out = jdbcTemplate.update(query, args);

			System.out.println("Saved to db, result = "+out);
			if (out != 0) {
